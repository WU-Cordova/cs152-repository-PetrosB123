...
class CircularQueue(IQueue[T]):
        ...
        if isinstance(item, self.__data_type):
            if not self.full:
                if self.__rear >= self.__max_size:
                    self.__rear = 0  # Wrap around
                if self.__rear < len(self.array):
                    self.array[self.__rear] = item
                else:
                    self.array.append(item)
                self.__rear = (self.__rear + 1) % self.__max_size
                self.__count += 1
            else:
                self.array[self.__front] = item
                self.__front = (self.__front + self.__count) % self.__max_size
                self.__rear = (self.__rear + self.__count) % self.__max_size
        else:
    ...
    def dequeue(self) -> T:
        ...
        '''
        if not self.empty:
            item = self.array[self.__front]
            self.__front = (self.__front + 1) % self.__max_size
            self.__count -= 1
            return item
        else:
            raise IndexError("Circular queue is empty, nothing to dequeue")
    ...
    def front(self) -> T:
        ''' Returns the item at the front of the queue without removing it
            ...
            Raises:
                IndexError: If the queue is empty
        '''
        return self.array[self.__front]
    ...
    def empty(self) -> bool:
        ...
        '''
        return all(item == None for item in self.array)
    ...
    def __eq__(self, other: object) -> bool:
        ...
        '''
        if isinstance(other, CircularQueue):
            if len(other) == len(self) and self.__data_type == other.__data_type:
                for i in range(len(self)):
                    self_index = (self.__front + i) % self.__max_size
                    other_index = (other.__front + i) % other.__max_size
                    if self.array[self_index] != other.array[other_index]:
                        return False
                return True
            else:
                return False
